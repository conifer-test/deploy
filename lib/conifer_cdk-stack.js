...
class ConiferCdkStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    const { ec2InstanceType, appImage, taskMemoryLimit, tests, bucketName } =
      props.env;
    ...
    const autoScalingGroup = cluster.addCapacity(
      'ConiferAutoScalingGroupCapacity',
      {
        ...
        desiredCapacity: 1,
        keyName: 'test-parallel-key', // TODO: Remove this later
    ...
    const dynamoTable = new dynamodb.Table(this, 'Table', {
      ...
      sortKey: { name: 'testRunID', type: dynamodb.AttributeType.STRING },
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });

    dynamoTable.grantFullAccess(autoScalingGroup);
    ...
    const myClusterName = cluster.clusterName;
    ...
    new cdk.CfnOutput(this, 'clusterName', { value: myClusterName });
